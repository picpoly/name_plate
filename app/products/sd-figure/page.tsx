...
import { Heart, Share2, Info, X, Check, Upload, Plus, Trash2 } from "lucide-react"
...
export default function SDFigurePage() {
  ...
  const [figureSize, setFigureSize] = useState("70mm")
  const [activeTab, setActiveTab] = useState("price")
  const [currentStep, setCurrentStep] = useState(1)

  // 見積もりフォーム関連の状態
  const [isQuoteFormOpen, setIsQuoteFormOpen] = useState(false)
  const [quoteFormStep, setQuoteFormStep] = useState(1) // 1: 顧客情報, 2: 完了
  ...
  const [figureColors, setFigureColors] = useState([
    { id: uuidv4(), texture: "normal", color: "#FFFFFF", colorName: "ジェイドホワイト", partName: "肌" },
    { id: uuidv4(), texture: "normal", color: "#000000", colorName: "ブラック", partName: "髪" },
    { id: uuidv4(), texture: "normal", color: "#0A2989", colorName: "ブルー", partName: "服" },
    { id: uuidv4(), texture: "normal", color: "#F55A74", colorName: "ピンク", partName: "靴" },
  ])
  ...
  // 見積もりフォームを開いたときに見積もりIDを生成
  useEffect(() => {
    if (isQuoteFormOpen && !quoteId) {
      const newQuoteId = `Q-${Date.now().toString().slice(-6)}-${Math.floor(Math.random() * 1000)}`
      setQuoteId(newQuoteId)
    }
  }, [isQuoteFormOpen, quoteId])

  // LINE連携のためのリダイレクト処理
  useEffect(() => {
    if (quoteFormStep === 2 && contactMethod === "line") {
      const timer = setTimeout(() => {
        window.location.href = "https://xanlic6j.autosns.app/addfriend/s/vf0PSX2buB/@490ncwxl"
      }, 3000) // 3秒後にリダイレクト

      return () => clearTimeout(timer)
    }
  }, [quoteFormStep, contactMethod])

  // 価格表（ボリュームディスカウント）
  const priceTable = [
    { quantity: 1, totalPrice: 25000, unitPrice: 25000 },
    { quantity: 2, totalPrice: 32000, unitPrice: 16000 },
    { quantity: 3, totalPrice: 39000, unitPrice: 13000 },
    { quantity: 4, totalPrice: 46000, unitPrice: 11500 },
    { quantity: 5, totalPrice: 53000, unitPrice: 10600 },
    { quantity: 6, totalPrice: 57500, unitPrice: 9583 },
    { quantity: 7, totalPrice: 64000, unitPrice: 9143 },
    { quantity: 8, totalPrice: 70500, unitPrice: 8812 },
    { quantity: 9, totalPrice: 77000, unitPrice: 8556 },
    { quantity: 10, totalPrice: 81700, unitPrice: 8170 },
    { quantity: 20, totalPrice: 144700, unitPrice: 7235 },
    { quantity: 30, totalPrice: 201700, unitPrice: 6723 },
    { quantity: 50, totalPrice: 297700, unitPrice: 5954 },
  ]
  ...
  // 商品画像の配列
  const productImages = [
    { src: "/sd-figure-new.png", alt: "人物SDフィギュア例 1" },
    { src: "/pink-hair-sd-figure.png", alt: "ピンク髪SDフィギュア例" },
  ]

  // 現在表示中のメイン画像を管理するステート
  const [currentMainImage, setCurrentMainImage] = useState("/sd-figure-new.png")
  ...
  const getCurrentPrice = () => {
    // 数量に基づいて価格を取得
    let unitPrice = priceTable[0].unitPrice

    // 数量に一致するエントリを探す
    const exactMatch = priceTable.find((entry) => entry.quantity === quantity)
    if (exactMatch) {
      unitPrice = exactMatch.unitPrice
    } else if (quantity > 50) {
      // 50個以上の場合は50個の単価を使用
      unitPrice = priceTable[priceTable.length - 1].unitPrice
    } else {
      // 該当するエントリがない場合は近い数量の価格を使用
      for (let i = 0; i < priceTable.length; i++) {
        if (quantity < priceTable[i].quantity) {
          if (i > 0) {
            unitPrice = priceTable[i - 1].unitPrice
          }
          break
        }
      }
  ...
  const openQuoteForm = () => {
    // 画像アップロードのバリデーション
    if (!uploadedFile) {
      setUploadError("フィギュア画像のアップロードは必須です。")
      // アップロードセクションまでスクロール
  ...
  const submitQuoteRequest = async () => {
    ...
    if (contactMethod === "email") {
      ...
      if (!contactLine) {
        setContactError("LINE IDを入力してください。")
        return
      }
    }
    ...
    const quoteData = {
      ...
      productDetails: {
        ...
        baseTexture: baseOption !== "none" ? baseTexture : "なし",
        notes,
      },
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || "見積もり依頼の送信に失敗しました")
      }

      const data = await response.json()
      console.log("見積もり依頼の送信に成功しました:", data)

      // 送信成功
      setSubmitSuccess(true)
      setQuoteFormStep(2)
    } catch (error) {
      console.error("見積もり依頼の送信に失敗しました", error)
      setContactError("見積もり依頼の送信に失敗しました。もう一度お試しください。")
    } finally {
  ...
  // LINE公式アカウントに接続する関数
  const connectToLine = () => {
    // 実際の実装ではLINE公式アカウントのURLに遷移する
    window.open("https://line.me/R/ti/p/@yourcompany", "_blank")
  }
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <div className="container mx-auto px-4 py-8 flex-grow">
        <div className="flex flex-col md:flex-row gap-8">
          ...
          {/* 商品情報 */}
          <div className="w-full md:w-1/2">
            <h1 className="text-3xl font-bold mb-2">人物SDフィギュア</h1>

            <Alert className="mb-4">
              <AlertCircle className="h-4 w-4" />
              <AlertTitle>見積もり依頼について</AlertTitle>
              <AlertDescription>
                人物SDフィギュアは見積もりが必要な商品です。小物や装飾が多い場合は追加料金が発生する場合があります。
                詳細をご入力いただき、見積もり依頼をお願いいたします。
              </AlertDescription>
            </Alert>

            <div className="mb-4">
              <Badge className="bg-green-100 text-green-800 hover:bg-green-100">30〜60営業日で発送</Badge>
            ...
            <p className="text-sm text-muted-foreground mb-4">送料：無料</p>
            ...
            <div className="space-y-6 mb-6">
                ...
                <div>
                  <div className="flex items-center mb-2">
                    ...
                    <InfoPopover name="size" title="フィギュアサイズ">
                      ...
                      <p>標準サイズは7cm×7cm×7cmです。</p>
                      <p>大きいサイズほど細部の表現が可能になります。</p>
                      <p>※一番サイズの大きい辺に合わせて調整されます。</p>
                    </InfoPopover>
                  </div>
                  <RadioGroup defaultValue="7cm" className="grid grid-cols-2 gap-4" onValueChange={setFigureSize}>
              ...
              {currentStep >= 4 && (
                <div className={`border rounded-lg p-6 ${currentStep === 4 ? "bg-blue-50" : ""}`}>
                  <StepHeader
                    number={4}
                    title="キャラクター画像をアップロード"
                    description="フィギュア化したいキャラクターの画像をアップロードしましょう。"
                  />
                  ...
                  <div id="figure-upload-section">
                    <div className="flex items-center mb-2">
                      <h3 className="font-medium">
                        キャラクター画像をアップロード<span className="text-red-500 ml-1">*</span>
                      </h3>
                      <InfoPopover name="upload" title="キャラクター画像">
                        <p>フィギュア化したいキャラクターの画像をアップロードしてください。</p>
              ...
              {currentStep >= 5 && (
                <div className={`border rounded-lg p-6 ${currentStep === 5 ? "bg-blue-50" : ""}`}>
                  <StepHeader
                    number={5}
                    title="注文内容の確認"
                    description="選択内容と金額を確認して、見積もりを依頼しましょう。"
                  />
                  ...
                  <div className="bg-gray-50 p-4 rounded-lg border mb-6">
                    ...
                    <ul className="space-y-2 text-sm">
                      ...
                      <li>
                        <span className="font-medium">フィギュアサイズ:</span> {figureSize}
                      </li>
                      <li>
                        ...
                        <ul className="ml-4 mt-1">
                          {figureColors.map((color) => (
                            <li key={color.id}>
                              {color.partName}: {color.colorName}（
                      ...
                      <li>
                        <span className="font-medium">キャラクター画像:</span>{" "}
                  ...
                  <div className="bg-gray-50 p-4 rounded-lg border mb-6">
                    <div className="flex justify-between items-center">
                      <span className="text-lg">参考価格:</span>
                    ...
                    <p className="text-sm text-amber-600 mt-2">
                      ※小物や装飾が多い場合は追加料金が発生する場合があります。正確な価格は見積もり後にご案内いたします。
                    </p>
                  </div>

                  <div className="flex gap-4">
                    <Button className="flex-1" size="lg" onClick={openQuoteForm}>
                      見積もり依頼をする
                    </Button>
            ...
            <Tabs id="product-tabs" defaultValue="price" value={activeTab} onValueChange={setActiveTab}>
              ...
              <TabsContent value="price" className="pt-4">
                ...
                <p className="text-sm text-gray-600 mt-4">※51体以上ご注文の方はご相談ください</p>
                <p className="text-sm text-gray-600 mt-2">
                ...
                <Table>
                  <TableBody>
                    ...
                    <TableRow>
                      ...
                      <TableCell>7cm×7cm×7cm〜10cm×10cm×10cm（選択可能）</TableCell>
                    </TableRow>
                    <TableRow>
                      <TableCell className="font-medium">重量</TableCell>
                      <TableCell>約50g〜150g（サイズによる）</TableCell>
                    </TableRow>
                    <TableRow>
                      <TableCell className="font-medium">包装</TableCell>
                      <TableCell>専用ボックス入り</TableCell>
                    </TableRow>
                    <TableRow>
                      <TableCell className="font-medium">彩色</TableCell>
                      <TableCell>高品質アクリル塗料使用</TableCell>
                    </TableRow>
                  </TableBody>
                </Table>
              </TabsContent>
              <TabsContent value="details" className="pt-4">
                <ul className="list-disc pl-5 space-y-2 text-sm">
                  <li>オリジナルキャラクターを立体化</li>
                  <li>アニメ・漫画風のデフォルメスタイル</li>
                  <li>高品質な3Dプリント技術で細部まで再現</li>
                  <li>丈夫で長持ちする素材を使用</li>
                  <li>サイズは最大10cm×10cm×10cm以内（選択可能）</li>
                  <li>記念品やプレゼントに最適</li>
                  <li>コレクションとしても人気</li>
                ...
                <div className="mt-6 grid grid-cols-2 gap-4">
                  <div className="relative aspect-square rounded overflow-hidden">
                    <Image src="/sd-figure-new.png" alt="SDフィギュア例" fill className="object-contain" />
                  </div>
                  <div className="relative aspect-square rounded overflow-hidden">
                    <Image src="/pink-hair-sd-figure.png" alt="SDフィギュア例2" fill className="object-contain" />
                  </div>
                </div>
              </TabsContent>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          <Link href="/products/pet-figure" className="block group">
            <div className="bg-white rounded-md overflow-hidden shadow-sm hover:shadow-md transition-shadow">
              <div className="relative aspect-square w-full">
                <Image
                  src="/realistic-animal-figure.png"
                  alt="ペットフィギュア"
                  fill
                  className="object-contain p-2 group-hover:scale-105 transition-transform duration-300"
                />
      ...
      {/* 見積もりフォームダイアログ */}
      <Dialog open={isQuoteFormOpen} onOpenChange={closeQuoteForm}>
        <DialogContent className="sm:max-w-[500px]">
          <DialogHeader>
            <DialogTitle>
              {quoteFormStep === 1 ? "見積もり依頼 - お客様情報入力" : "見積もり依頼 - 送信完了"}
            </DialogTitle>
            <DialogDescription>
              {quoteFormStep === 1
                ? "お客様の連絡先情報をご入力ください。見積もり結果をご連絡いたします。"
                : "見積もり依頼を受け付けました。担当者より連絡いたします。"}
            </DialogDescription>
          </DialogHeader>
                  ...
                  <div className="col-span-3">
                    ...
                    >
                      ...
                      <div className="flex items-center space-x-2">
                        ...
                        <Label htmlFor="contact-line" className="flex items-center">
                          LINE
            ...
            <>
              <div className="py-6 flex flex-col items-center justify-center">
                ...
                <p className="text-center mb-4">
                  見積もり依頼を受け付けました。内容を確認後、担当者より{contactMethod === "email" ? "メール" : "LINE"}
                  にてご連絡いたします。
                </p>
                <p className="text-sm text-gray-500 text-center">
                  見積もりID: <span className="font-mono">{quoteId}</span>
                </p>
                ...
                {contactMethod === "line" && (
                  <div className="mt-4 p-4 bg-green-50 border border-green-200 rounded-md w-full">
                    <p className="text-center text-green-700 font-medium mb-2">
                      数秒後に自動で公式LINEにリダイレクトします
                    </p>
                    <div className="flex justify-center">
                      <Image src="/line-logo.png" alt="LINE" width={60} height={60} />
                    </div>
                  </div>
                )}
              </div>
              <DialogFooter>
                <Button onClick={closeQuoteForm}>閉じる</Button>
              </DialogFooter>
            </>
          )}
        </DialogContent>
